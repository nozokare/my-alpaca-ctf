...
いままで bot が Cookie にフラグを保存してアクセスするタイプの XSS 問題は、XSS ができたとして最終的にフラグを受け取る手段が分からなくて解けずにいたのですが、今回の問題は説明文に案内があったので、解くことができました。

最近まで webhook.site というサービスの存在を知らず、AlpacaHack で完結せずに自前のサーバーがないと解けない問題が出題されるものなのか？と思い込んでしまっていたので、他の XSS 問題にも同様の案内があるといいなと思いました。
