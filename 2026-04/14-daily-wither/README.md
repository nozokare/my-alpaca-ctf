...
- 確率 1/2 で $C_i = P_i\ \&\ 0 = 0$
- 確率 1/2 で $C_i = P_i\ \&\ 1 = P_i$

になります。

$n$ 個の cipher を取得したとき、各 chiper の $i$ 番目のビット $C_i^{(1)}, C_i^{(2)}, \ldots, C_i^{(n)}$ が全て $P_i\ \&\ 0$ である確率は $1/2^n$ です。

したがって、十分な数の cipher を取得すれば高い確率でどれかは $P_i\ \&\ 1$ を含むことになり、このとき
